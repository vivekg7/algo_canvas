...
class GameOfLifeAlgorithm extends Algorithm {
  ...
  AlgorithmState createInitialState() {
    ...
    final liveCells = _countLive(grid, rows, cols);
  ...
  AlgorithmState? tick(AlgorithmState current) {
    final s = current as GameOfLifeState;
    final rows = s.rows;
    final cols = s.cols;
    final grid = s.grid;
    ...
    final liveCells = _countLive(newGrid, rows, cols);
    ...
    return GameOfLifeState(
      ...
      born: born,
      died: died,
      description: 'Generation $gen: $liveCells live cells',
    );
  }
