...
class BfsAlgorithm extends Algorithm {
  ...
  String get description =>
      'Breadth-First Search explores all neighbors before going deeper. O(V+E).';
  ...
  Future<List<AlgorithmState>> generate() async {
    ...
    queue.add(0);
    visited.add(0);
    currentNodes[0] = currentNodes[0].copyWith(status: NodeStatus.queued);
    ...
    while (queue.isNotEmpty) {
      ...
      for (final neighbor in adj[node]!) {
        ...
        if (!visited.contains(neighbor)) {
          ...
          queue.add(neighbor);
          currentNodes[neighbor] = currentNodes[neighbor].copyWith(status: NodeStatus.queued);
    ...
    states.add(GraphState(
      nodes: List.of(currentNodes),
      edges: List.of(currentEdges),
      description: 'BFS complete — ${visited.length} nodes visited',
    ));
    ...
    return Row(
      children: [
        ...
        Expanded(child: Slider(
          value: _value, min: 5, max: 25, divisions: 20,
